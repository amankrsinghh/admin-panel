...
class Collection<T> {
  ...
  async add(item: T): Promise<T> {
    if (isFirebaseEnabled) {
      const data = { ...item } as any;
      const id = data[this.idField] || Math.random().toString(36).slice(2, 10).toUpperCase();
  ...
  async remove(id: string): Promise<void> {
    if (isFirebaseEnabled) {
      ...
      const list = this.all().filter((x) => this.idOf(x) !== id);
      localStorage.setItem(this.key, JSON.stringify(list));
      this.emit();
    }
  }
...
// ---------- Order placement that also pushes to Google Sheets ----------
export async function placeOrder(o: Order): Promise<Order> {
  ordersDB.add(o);
  // Fire-and-forget — sheet sync should never block UX
  pushOrderToSheet(o).catch(() => void 0);
  return o;
}
